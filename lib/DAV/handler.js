...
(function() {
    ...
    this.invoke = function() {
        ...
        if (jsDAV.debugMode) {
            ...
            console.log("INFO: invoking method '" + method + "'");
            ...
            this.httpResponse.end = function(content) {
                console.log("request end: ", _self.id);
        ...
        this.dispatchEvent("beforeMethod", method, function(stop) {
            ...
            if (internalMethods[method]) {
                var result = _self["http" + method.charAt(0) + method.toLowerCase().substr(1)]();
		_self.dispatchEvent("afterMethod",method, function(stop2){});
	        return result;
            }
            else {
                ...
                function afterGetProperty(rprop, rpath, remRT, newProps, cbnext) {
                    ...
                    _self.dispatchEvent("afterGetProperties", rpath, newProps, function() {
                        if (parentNode.hasFeature(jsDAV.__ICOLLECTION__)) {
                            ...
                            rpath = s.indexOf(_self.server.baseUri) !== 0 ? rpath.replace(s, "").replace(/^[\/]+/, "") : rpath;
                        }
                        newProps["href"] = rpath;
